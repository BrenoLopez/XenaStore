      ...
      <div class="row">
        <div class="col-12">
          <?php
            ...
            $iditem = filter_var($_GET['iditem'],FILTER_VALIDATE_INT);
            $nomeProduto= $_GET['nomeProduto'];
          $consulta = $conectar->query("SELECT * FROM items where iditem='$iditem'");
            $linha1 = $consulta->fetch(PDO::FETCH_ASSOC);
          ?>
          <form action="../../php/sistema/editaItem.php" method="post" enctype="multipart/form-data">
            <!-- area de campos do form -->
            <hr/>
            <div class="row">
              <div class="form-group col-md-2">
                <label>Código Produto</label>
                <input type="number" class="form-control" name="iditem" id="iditem" value="<?php echo $iditem ?>" readonly="readonly">
              </div>
              <div class="form-group col-md-4">
                <label>Nome Produto</label>
                <input type="text" class="form-control" name="nomeproduto" id="nomeproduto" value="<?php echo $linha1['nomeproduto'] ?>" required>
              </div>
              <div class="form-group col-md-3">
                <label>Valor Compra</label>
                <input type="text" class="form-control" name="precocompra" id="precocompra" value="<?php echo $linha1['precocompra'] ?>" required>
              </div>
              <div class="form-group col-md-3">
                <label>Valor Venda</label>
                <input type="text" class="form-control" name="precovenda" id="precovenda" value="<?php echo $linha1['precovenda'] ?>" required>
              </div>
            </div>
            <div class="row">
                 <div class="form-group col-md-12">
                <label for="descricao">Descrição</label>
                <input type="text" class="form-control" name="descricao" id="descricao" value="<?php echo $linha1['descricao'] ?>" required>
              </div>
            </div>
            ...
            <div class="row">
              <div class="form-group col-md-6">
                <label for="imagem">Imagem</label>
                  <input type="file" class="form-group" name="imagem"><?php echo "<br>foto salva:".$nomeProduto ;?>
              </div>
              <div class="form-group col-md-6">
                ...
                <select name="idcategoria" required>"
                  <?php
                  include_once "../../php/conexao.php";
                  $consulta = $conectar->query("SELECT idcategoria,nomecategoria
                                                FROM categoria");
